...
public interface NetworkInterface {
    ...
    /*@ public normal_behavior
      ...
      @ assignable articles, accounts.get(accountId).articles, accounts.get(accountId).followers[*].articles;
      @ ensures containsArticle(articleId) && accounts.get(accountId).containsArticle(articleId);
      @ ensures (\forall PersonInterface p; accounts.get(accountId).followers.hasPerson(p); p.getReceivedArticles().get(0).equals(articleId));
      @ ensures (\forall PersonInterface p; accounts.get(accountId).followers.hasPerson(p); (\forall int i; 0 <= i && i < \old(p.getReceivedArticles().size());
      @                                                                         p.getReceivedArticles().get(i+1) == \old(p.getReceivedArticles().get(i))));
      @ ensures (\forall PersonInterface p; accounts.get(accountId).followers.hasPerson(p); p.getReceivedArticles().size() == \old(p.getReceivedArticles().size() + 1));
      @ also
      @ public exceptional_behavior
      @ signals (PersonIdNotFoundException e) !containsPerson(personId);
      @ signals (OfficialAccountIdNotFoundException e) containsPerson(personId) &&
    ...
    /*@ public normal_behavior
      ...
      @ ensures (\forall PersonInterface p; accounts.get(accountId).followers.hasPerson(p); (\forall int i; 0 <= i && i < (p.getReceivedArticles().size());
      @                                                                         p.getReceivedArticles().get(i) != articleId));
      @ ensures (\exists int i; 0 <= i && i < accounts.get(accountId).followers.length; accounts.get(accountId).followers[i] == getPerson(personId) && accounts.get(accountId).contributions[i] == \old(accounts.get(accountId).contributions[i]) - 1);
      @ also
      @ public exceptional_behavior
      @ signals (PersonIdNotFoundException e) !containsPerson(personId);
    ...
    /*@ public normal_behavior
      ...
      @          containsAccount(accountId) &&
      @          !accounts.get(accountId).containsFollower(getPerson(personId));
      @ assignable accounts.get(accountId).followers;
